...
#
##################################################################
workers <- startWorkers(5) # My computer has 2 cores
...
system.time(x <- foreach(j=1:5 ) %dopar% partree(j))  #  2.56 seconds  (notice that the first run would be slower, because of R's lazy loading)
system.time(for(j in 1:5 ) x <- partree(j))  #  4.82 seconds
